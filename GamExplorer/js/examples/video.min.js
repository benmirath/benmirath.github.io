...
(function(window, undefined) {
    var document = window.document, CDN_VERSION = "3.0";
    ...
    _V_.extend({
        ...
        formatTime: function(seconds, guide) {
            var guide = guide || seconds, s = Math.floor(seconds%60), m = Math.floor(seconds / 60%60), h = Math.floor(seconds / 3600), gm = Math.floor(guide / 60%60), gh = Math.floor(guide / 3600);
            h = (h > 0 || gh > 0) ? h + ":" : "";
    ...
    _V_.Slider = _V_.Component.extend({
        init: function(player, options) {
            ...
            _V_.each.call(this, this.components, function(comp) {
                if (comp instanceof _V_[this.barClass]) {
                    ...
                    if (comp instanceof _V_[this.handleClass]) {
                        this.handle = comp
                    }
                }
            });
        ...
        update: function() {
            ...
            if (handle) {
                var box = this.el, boxWidth = box.offsetWidth, handlePercent = (handle.el.offsetWidth) ? handle.el.offsetWidth / boxWidth: 0, boxAdjustedPercent = 1 - handlePercent;
    ...
    _V_.Player = _V_.Component.extend({
        init: function(tag, addOptions, ready) {
            this.tag = tag;
            var el = this.el = _V_.createElement("div"), options = this.options = {}, width = options.width = tag.width, height = options.height = tag.height, initWidth = width || 300, initHeight = height || 150;
        ...
        buffered: function() {
            var buffered = this.apiCall("buffered"), start = 0, end = this.values.bufferEnd = this.values.bufferEnd || 0, timeRange;
        ...
        volume: function(percentAsDecimal) {
            if (percentAsDecimal !== undefined) {
                var vol = Math.max(0, Math.min(1, parseFloat(percentAsDecimal)));
    ...
    _V_.flash = _V_.PlaybackTech.extend({
        init: function(player, options) {
            ...
            }), objId = player.el.id + "_flash_api", playerOptions = player.options;
            flashVars = _V_.merge({
        ...
        update: function() {
            if (this.cues && this.cues.length > 0) {
                ...
                if (!this.currentCue || this.currentCue.startTime >= time || this.currentCue.endTime < time) {
                    var newSubIndex = false, reverse = (this.cues[this.lastCueIndex].startTime > time), i = this.lastCueIndex - (reverse ? 1 : 0);
                    while (true) {
                        if (reverse) {
